...
export const CmsEditable = <CT extends EditableComponentType = 'div'>({ as : DefaultElement, cmsId, cmsFieldName, children, key, ...props }: CmsEditableProps<CT>) =>
{
    const HtmlElement = DefaultElement ?? 'div' as CT
    const { inEditMode } = getServerContext()
    return <HtmlElement {...props} data-epi-block-id={ inEditMode ? cmsId ?? undefined : undefined } data-epi-property-name={ inEditMode ? cmsFieldName ?? undefined : undefined }>{ children }</HtmlElement>
}
