import type { ComponentFactory, ComponentType, ComponentTypeHandle, ComponentTypeDictionary, ComponentTypeDictionaryEntry } from './types.js'
...
export class DefaultComponentFactory implements ComponentFactory {
    ...
    register(type: ComponentTypeHandle, component: ComponentType, useSuspense: boolean = false, loader?: ComponentType) : void
    {
        const registryKey = processComponentTypeHandle(type)
        if (this.dbg) console.log(`➕ [DefaultComponentFactory] Adding ${ registryKey }`)
    ...
    {
        const registryKey = processComponentTypeHandle(type)
        if (this.dbg) console.log(`⚡ [DefaultComponentFactory] Resolving ${ registryKey }`)
        const entry = this.registry.get(registryKey)
        if (!entry) return undefined // The key is not registered in the factory
        ...
        function Suspended(props: Record<string,any>) {
            return <Suspense fallback={ EntryLoader && <EntryLoader { ...props } /> } >
