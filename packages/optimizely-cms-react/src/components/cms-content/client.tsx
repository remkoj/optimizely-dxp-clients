...
export function CmsContent<LocalesType = string>({
  ...
  const [myContentType, setMyContentType] = useState<ContentType | undefined>(
    resolveContentType(contentType, fragmentData)
  )
  const contentLink = useMemo(
    () => normalizeContentLink(rawContentLink),
    [rawContentLink]
  )
  const Component = useMemo(
    () => resolveComponent(myContentType, contentTypePrefix, variant, ctx),
    [myContentType, variant, contentTypePrefix, ctx]
  )
  ...
  if (graphClient?.debug) {
    ...
    if (
      ...
      console.warn(
        `🟠 [CmsContent] ${mode} mode active without an authenticated graphClient, this will cause problems. Make sure the context has an authenticated client.`
      )
  }
  ...
  useEffect(() => {
    ...
    if (myContentType || !contentLink) return

    if (!graphClient) {
      console.warn(
        `🔴 [CmsContent] No Optimizely Graph Client present unable to load content type`
      )
      return
    }
    ...
    if (graphClient.debug)
      console.warn(
        `🟠 [CmsContent] No content type provided for content ${contentLinkToString(contentLink)}, this causes an additional GraphQL query to resolve the myContentType`
      )
  ...
  useEffect(() => {
    ...
    if (noDataLoad || !contentLink || !Component) return
    ...
    //@ts-expect-error
    getContent(graphClient, contentLink, Component, fragmentData).then(
      (newData) => {
  ...
  return (
    <Component
      contentLink={contentLink as ContentLink}
      data={data}
