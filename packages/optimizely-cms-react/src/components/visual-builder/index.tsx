 ...
 * @returns     The
 */
export function OptimizelyComposition({ node, leafPropsFactory = defaultPropsFactory, nodePropsFactory = defaultNodePropsFactory, ctx, cmsContent: CmsContent }: BaseOptimizelyCompositionProps) : ReactNode
{
    ...
    return <CmsContent contentType={contentType} contentLink={contentLink} fragmentData={fragmentData} layoutProps={layoutProps} noDataLoad>
        {(node.nodes ?? []).map((child) => {
            ...
            return <OptimizelyComposition key={ childKey } node={ child } leafPropsFactory={ leafPropsFactory } nodePropsFactory={ nodePropsFactory } ctx={ ctx } cmsContent={ CmsContent }/>
        })}
    </CmsContent>
}
