...
export const hasStyleFilter: (args: ArgumentsCamelCase<OptiCmsArgs<StylesArgs>>) => boolean = (args) => {
  const { excludeBaseTypes, excludeTypes, excludeNodeTypes, excludeTemplates, baseTypes, types, nodes, templates, templateTypes } = parseArgs(args);
  const hasTemplateFilter = excludeTemplates.length > 0 || templates.length > 0 || templateTypes.length > 0;
  const hasBaseTypeFilter = excludeBaseTypes.length > 0 || baseTypes.length > 0;
  const hasNodeFilter = excludeNodeTypes.length > 0 || nodes.length > 0;
  const hasTypeFilter = excludeTypes.length > 0 || types.length > 0;
  return hasTemplateFilter || hasBaseTypeFilter || hasNodeFilter || hasTypeFilter;
}
